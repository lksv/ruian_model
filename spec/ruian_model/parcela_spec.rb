  ...
  describe '#to_address' do
    ...
    context 'without "st." e.g. druh_cislovani_kod = 2' do
      it 'do not show "st."' do
        ...
        expect(subject.to_address).to eq 'parc.č. st. 204 (zast. pl.), k.u. Krahulov'
      end
    end
    ...
    context 'with "st." (druh_cislovani_kod = 1)' do
      ...
      it 'with poddeleni_cisla' do
        ...
        expect(subject.to_address).to eq 'parc.č. st. 301/2 (zast. pl.), k.u. Stařeč'
      end
    end
    ...
    describe 'druh pozemku' do
      it 'shows "druh pozemku" in brackets"' do
        ...
        expect(subject.to_address).to eq 'parc.č. 182/40 (orná půda), k.u. Řípov'
        subject.druh_pozemku_kod = 14
        expect(subject.to_address).to eq 'parc.č. 182/40 (ostat.pl.), k.u. Řípov'
      end
    end
  end
end
